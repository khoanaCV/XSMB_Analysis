...
const Banner = () => {
    ...
    const extractPrizes = (result) => {
        ...
        return [
            ...
            result.prize1,
            result.prize2_1, result.prize2_2,
            result.prize3_1, result.prize3_2, result.prize3_3, result.prize3_4, result.prize3_5, result.prize3_6,
            result.prize4_1, result.prize4_2, result.prize4_3, result.prize4_4,
            result.prize5_1, result.prize5_2, result.prize5_3, result.prize5_4, result.prize5_5, result.prize5_6,
            result.prize6_1, result.prize6_2, result.prize6_3,
            result.prize7_1, result.prize7_2, result.prize7_3, result.prize7_4
        ].flat().map(number => number.toString().padStart(2, '0')); // Sử dụng .flat() để làm phẳng mảng trong trường hợp các giải có nhiều hơn một số
    };
    ...
    const handleSearch = () => {
        if (Array.isArray(lotteryResults)) {
            const filteredResults = lotteryResults.filter(result => {
                const resultDate = utcToZonedTime(parseISO(result.draw_date), Intl.DateTimeFormat().resolvedOptions().timeZone);
        ...
        <section className=" text-black d-flex justify-content-center align-items-center">
            <Container>
                <Row className="align-items-center">
                    ...
                    <Col md={12} sm={12} lg={6}>
                        <div className="hero-slide-left justify-content-end align-items-center text-center text-lg-start"> <br></br> <br></br>
                            ...
                            <div>
                                <div class="input-group mb-3">
                                    ...
                                    <h5>Đến ngày&nbsp;</h5> <input type="date" onChange={handleDateChange} />&nbsp;
                                    <input type="checkbox"></input> <h5>&nbsp;Bảng đầu đuôi</h5> &nbsp;
                                    <input type="checkbox"></input> <h5>&nbsp;Chỉ hiện giải ĐB</h5>
                                    <button type="button" class="btn btn-primary" onClick={handleSearch}>Xem kết quả</button>
                            ...
                            <td valign="top">
                                ...
                                {displayResults.map((result, index) => (
                                    <table key={index} className="table table-bordered text-black text-center" cellSpacing="1" cellPadding="9">
                                        <thead>
                                            <tr>
                                        ...
                                        <tbody>
                                            <tr>
                                                ...
                                                <td colSpan="12" className="kqcell kq_0 text-danger">{result.special_prize}</td>
                                            </tr>
                                            <tr>
                                                <td className="leftcol">Nhất</td>
                                                <td colSpan="12" className="kqcell kq_1">{result.prize1}</td>
                                            </tr>
                                            <tr>
                                                <td className="leftcol">Nhì</td>
                                                <td colSpan="6" className="kqcell kq_2">{result.prize2_1}</td>
                                                <td colSpan="6" className="kqcell kq_3">{result.prize2_2}</td>
                                            </tr>
                                            <tr>
                                                <td rowSpan="2" className="leftcol">Ba</td>
                                                <td colSpan="4" className="kqcell kq_4">{result.prize3_1}</td>
                                                <td colSpan="4" className="kqcell kq_5">{result.prize3_2}</td>
                                                <td colSpan="4" className="kqcell kq_6">{result.prize3_3}</td>
                                            </tr>
                                            <tr>
                                                <td colSpan="4" className="kqcell kq_7">{result.prize3_4}</td>
                                                <td colSpan="4" className="kqcell kq_8">{result.prize3_5}</td>
                                                <td colSpan="4" className="kqcell kq_9">{result.prize3_6}</td>
                                            </tr>
                                            <tr>
                                                <td className="leftcol">Tư</td>
                                                <td colSpan="3" className="kqcell kq_10">{result.prize4_1}</td>
                                                <td colSpan="3" className="kqcell kq_11">{result.prize4_2}</td>
                                                <td colSpan="3" className="kqcell kq_12">{result.prize4_3}</td>
                                                <td colSpan="3" className="kqcell kq_13">{result.prize4_4}</td>
                                            </tr>
                                            <tr>
                                                <td rowSpan="2" className="leftcol">Năm</td>
                                                <td colSpan="4" className="kqcell kq_14">{result.prize5_1}</td>
                                                <td colSpan="4" className="kqcell kq_15">{result.prize5_2}</td>
                                                <td colSpan="4" className="kqcell kq_16">{result.prize5_3}</td>
                                            </tr>
                                            <tr>
                                                <td colSpan="4" className="kqcell kq_17">{result.prize5_4}</td>
                                                <td colSpan="4" className="kqcell kq_18">{result.prize5_5}</td>
                                                <td colSpan="4" className="kqcell kq_19">{result.prize5_6}</td>
                                            </tr>
                                            <tr>
                                                <td className="leftcol">Sáu</td>
                                                <td colSpan="4" className="kqcell kq_20">{result.prize6_1}</td>
                                                <td colSpan="4" className="kqcell kq_21">{result.prize6_2}</td>
                                                <td colSpan="4" className="kqcell kq_22">{result.prize6_3}</td>
                                            </tr>
                                            <tr>
                                                <td className="leftcol">Bảy</td>
                                                <td colSpan="3" className="kqcell kq_23">{result.prize7_1}</td>
                                                <td colSpan="3" className="kqcell kq_24">{result.prize7_2}</td>
                                                <td colSpan="3" className="kqcell kq_25">{result.prize7_3}</td>
                                                <td colSpan="3" className="kqcell kq_26">{result.prize7_4}</td>
                                            </tr>
                                        ...
                                        </tr>
                                        {
                                            Object.keys(headsTable).map(head => (
                                                <tr key={head}>
                                                    ...
                                                    <td>
                                                        {
                                                            headsTable[head].map(number => number.slice(-1)).join(', ')
                                                        }
                                                    </td>
                                                </tr>
                                            ))
                                        ...
                                        </tr>
                                        {
                                            Object.keys(tailsTable).map(tail => (
                                                <tr key={tail}>
                                                    ...
                                                    <td>
                                                        {
                                                            tailsTable[tail].map(number => number.length > 1 ? number.slice(-2, -1) : '0').join(', ')
                                                        }
                                                    </td>
                                                </tr>
                                            ))
