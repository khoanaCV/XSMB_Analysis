...
const Service = () => {
  ...
  const handleSubmit = (event) => { 
    ...
    axios
    .post("http://localhost:9999/sparses/date", {
      date,
      days,
      number
    })
  ...
  return (
    <section className="service-jsx overflow-x-scroll">
      ...
      <form className="flex justify-center mb-2" onSubmit={handleSubmit}>
        Số ngày:<input className="border-[1px] border-black ml-1 mr-4 rounded-sm" type="text" name="days"
          size="10"
          onChange={handleDays}
        />
        Ngày cuối cùng:<input className="border-[1px] border-black	ml-1 mr-4 rounded-sm" type="date" name="date"
          size="10"
          onChange={handleDate}
        />
        Số số muốn xem: <input className="border-[1px] border-black ml-1 mr-4 rounded-sm" type="text" name="number"
          size="2"
            ...
            <tbody>
              {array.map((item, index) => (
                <tr key={index}>
                  ...
                  {columns.map((column) => (
                    <td key={column} className={getTableSquareClass(item[column])} >{item[column]}</td>
                  ))}
                </tr>
              ))}
            </tbody>
