...
let simulatedData = Array(numSimulations).fill(0).map(() => {
    return _.mapValues(meanOccurrences, val => poissonRandom(val));
});

const simulatedMeanOccurrences = _.mapValues(simulatedData[0], (value, key) => {
...
const combinedMeanOccurrences = _.mapValues(meanOccurrences, (value, key) => {
...
const normalizedProbScore = _.mapValues(combinedMeanOccurrences, val => (val - minVal) / (maxVal - minVal));
...
const streakScoresCorrected = _.mapValues(nonAppearanceStreaks, (value, key) => {
...
const normalizedStreakScores = _.mapValues(streakScoresCorrected, val => (val - minStreakVal) / (maxStreakVal - minStreakVal));
...
const recentAppearances = _.mapValues(_.omit(data[0], 'draw_date'), (value, key) => {
    return _.sumBy(_.takeRight(data, 45), row => parseInt(row[key], 10));
});


const frequentScores = _.mapValues(recentAppearances, val => (val >= 15 ? 1 : 0));
...
const normalizedLoRoiProbabilities = _.mapValues(loRoiProbabilities, val => (val - minLoRoi) / (maxLoRoi - minLoRoi));
...
//Kiểu chuẩn hoá
const finalScoresCorrected = _.mapValues(normalizedProbScore, (value, key) => {
    return (value + normalizedStreakScores[key] + frequentScores[key] + normalizedLoRoiProbabilities[key]) / 4;
});
